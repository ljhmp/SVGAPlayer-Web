...
export class Player {
  ...
  constructor(container, self) {
    if (typeof wx !== 'undefined') {
      if (self) {
        this._container = wx.createSelectorQuery().select(container).in(self)
        this.ctx = wx.createCanvasContext(container, self)
      } else {
        this._container = wx.createSelectorQuery().select(container)
  ...
  setText(textORMap, forKey) {
    ...
    let color =
      (typeof textORMap === 'object' ? textORMap.color : '#000000') || '#000000'
  ...
  _init() {
    if (typeof wx !== 'undefined') {
      this._container.fields({ node: true, size: true }).exec((res) => {
        ...
        this._ctxWidth = res[0].width * dpr
        this._ctxHeight = res[0].height * dpr
  ...
  _doStart(range, reverse, fromFrame) {
    ...
    this._animator.onUpdate = (value) => {
      ...
      if (typeof this._onPercentage === 'function') {
        this._onPercentage(
          parseFloat(this._currentFrame + 1) /
            parseFloat(this._videoItem.frames)
        )
      }
    }
  ...
  _resize() {
    ...
    if (typeof wx !== 'undefined') {
      ...
      if (
        ...
        if (this._contentMode === 'Fill') {
          ...
          const translateY =
            (imageSize.height * scaleY - imageSize.height) / 2.0
          ...
          if (
            ...
            const translateY =
              (imageSize.height * scale - imageSize.height) / 2.0 +
              (targetSize.height - imageSize.height * scale) / 2.0
            ctx.setTransform(scaleX, 0, 0, scaleY, translateX, translateY)
          } else if (
            (imageRatio < viewRatio && this._contentMode === 'AspectFit') ||
            (imageRatio > viewRatio && this._contentMode === 'AspectFill')
            ...
            const translateX =
              (imageSize.width * scale - imageSize.width) / 2.0 +
              (targetSize.width - imageSize.width * scale) / 2.0
            const translateY =
              (imageSize.height * scale - imageSize.height) / 2.0
            ctx.setTransform(scaleX, 0, 0, scaleY, translateX, translateY)
          }
    ...
    if (this._drawingCanvas === undefined || asParent === true) {
      ...
      let targetSize = {
        width:
          this._container !== undefined ? this._container.clientWidth : 0.0,
        height:
          this._container !== undefined ? this._container.clientHeight : 0.0
      }
      let imageSize = this._videoItem.videoSize
      if (this._contentMode === 'Fill') {
  ...
  _update() {
    ...
    // if (this._drawingCanvas === undefined) {
    //   return
    // }
    this._resize()
