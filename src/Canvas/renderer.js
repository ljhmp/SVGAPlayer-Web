...
function wxCreateImage(arraybuffer, callback) {
  ...
  wx.getFileSystemManager().writeFile({
    ...
    success: () => {
      const img = createImage(`${wx.env.USER_DATA_PATH}/${randomName}`)
      img.onload = function () {
        callback(img)
...
export class Renderer {
  ...
  prepare() {
    ...
    if (
      this._owner._videoItem.images === undefined ||
      Object.keys(this._owner._videoItem.images).length == 0
    ) {
      this._bitmapCache = {}
      this._prepared = true
      return
  ...
  drawSprite(sprite, ctx, frameIndex) {
    ...
    let src =
      this._owner._dynamicImage[bitmapKey] ||
      this._bitmapCache[bitmapKey] ||
      this._owner._videoItem.images[bitmapKey]
    if (
      typeof src === 'string' &&
      (src.indexOf('http://') === 0 || src.indexOf('https://') === 0)
    ) {
      const imgTag = createImage(src)
  ...
  resetShapeStyles(ctx, obj) {
    ...
    if (styles && styles.stroke) {
      ctx.strokeStyle = `rgba(${parseInt(styles.stroke[0] * 255)}, ${parseInt(
        styles.stroke[1] * 255
      )}, ${parseInt(styles.stroke[2] * 255)}, ${styles.stroke[3]})`
    } else {
    ...
    if (styles && styles.fill) {
      ctx.fillStyle = `rgba(${parseInt(styles.fill[0] * 255)}, ${parseInt(
        styles.fill[1] * 255
      )}, ${parseInt(styles.fill[2] * 255)}, ${styles.fill[3]})`
    } else {
